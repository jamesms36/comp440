        ...
        for i in self.beliefs:
            if self.beliefs[i] != 0:
                ...
                for p in self.allPositions:
                    newBeliefs[p] += newPosDist[p] * self.beliefs[i]

        for i in newBeliefs:
            self.beliefs[i] = newBeliefs[i]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(numToPos[i % positions])
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        n = self.numParticles
        positions1 = random.choices(self.legalPositions, k=n)
        positions2 = random.choices(self.legalPositions, k=n)
        self.particles = [(positions1[i], positions2[i]) for i in range(n)]
