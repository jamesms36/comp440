...
class Grader:
    ...
    def requireIsGreaterThanOrEqualTo(self, greaterThanQuantity, predAnswer ):
        if predAnswer >= greaterThanQuantity:
            return self.assignFullCredit()
        else:
            return self.fail("Expected to be >= %f, but got %f" %
    ...
    def printException(self):
        ...
        tb = [item for item in traceback.extract_tb(exc_traceback) if not isTracebackItemGrader(item)]
